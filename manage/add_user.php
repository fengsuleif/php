...
<body>
	...
	<div class="ch-clearfix">
		...
		<form action="add_user.php?action=add" class="ch-form myForm" method="POST">
			<fieldset>
				...
				<div class="ch-form-row">
					...
					<input class="ch-form-ico-input" type="text" id="input_name" name="SXK_name" size="30" placeholder=""  >
...
<script type="text/javascript">
	       $(document).ready(function () {
            $("#select1").get(0).selectedIndex=(<?php  ?>);//设置修改内容时选取正确的分类           
            })
</script>
</html>
<?php	
$name=new_addslashes($_POST["SXK_name"]);//$_GET["UserName"];
$pw=md5($_POST["SXK_pw2"]);
$mail=$_POST["SXK_mail"];//$_GET["Email"];
$date=date("Y-m-d H:i:s" ,time()) ;	
$power=$_POST["SXK_power"];

if($_GET["action"]=='add'){
     if($_POST["SXK_pw1"]==$_POST["SXK_pw2"]  ){
            if(user_in($name) and mail_in($mail)){	
			
			add_user($name,$pw,$mail,$power,$date);	
		
			$url='http://'.$_SERVER["SERVER_NAME"].'/manage/list_user.php';
	       gourl($url);
			}else{echo "0添加失败，用户名已存在或邮箱已存在";}						
		}else{
		echo "2添加失败，两次密码输入不一致或邮箱格式错误";
		}
}
?>